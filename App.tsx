...
const App: React.FC = () => {
  ...
  // --- REPORT EXPORTERS (Keep existing implementation) ---
  const handleExportMaster = () => { /* ... existing code ... */ };
  const handleExportTeams = () => { /* ... existing code ... */ };
  const handleExportSports = () => { /* ... existing code ... */ };
  ...
  const handleUnsellPlayer = (playerId: number) => {
      ...
      // Optional: Add a 'revert' log only if we want a trace, but user asked to "Remove the entry".
      // We will skip adding a new log to strictly follow "Remove the entry" or we can add a subtle one. 
      // Let's NOT add a log to keep the history clean as if the mistake didn't happen.
  };
