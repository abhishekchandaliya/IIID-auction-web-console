  ...
  useEffect(() => {
    ...
    if (savedPlayers) {
        try {
            ...
            if (parsed.length > 0) {
                // Migration for legacy data (add status if missing, add auctionType if missing)
  ...
  const handleMasterLoad = (rawData: any[]) => {
    const parsedPlayers: Player[] = rawData
        ...
        .map((row: any, index: number) => {
            ...
            const pType = getRowValue(row, 'Auction Type', 'Type', 'Pool'); // New Column
                                ...
                                <div className="pt-4 border-t border-slate-700">
                                    ...
                                    {settingsSportTab && tempConfig.categoryLimits && tempConfig.sportLimits && (
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 bg-slate-900 p-6 rounded-xl border border-slate-800 animate-in fade-in">
                                            {/* Category Limits */}
                                            <div>
                                                <h5 className="text-xs font-bold text-indigo-400 uppercase mb-4 tracking-widest">Min Players per Grade</h5>
