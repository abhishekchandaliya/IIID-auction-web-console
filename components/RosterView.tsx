...
const RosterView: React.FC<RosterViewProps> = ({ 
  ...
  const handleExportTeam = (teamName: string) => {
      ...
      // 1. Master Roster Sheet (Detailed, with Price and All Sports)
      const masterData = teamPlayers.map(p => {
          const row: any = {
              "ID": p.id,
              "Player Name": p.name,
              "Price": p.price,
          ...
          // Add all dynamic sport columns
          sports.forEach(s => {
              row[`${s} Grade`] = p.ratings[s] || 'N/A';
          });
          return row;
      });
      ...
      // 2. Individual Sport Sheets (Filtered: Only players playing this sport)
      sports.forEach(sport => {
          ...
          // Create sheet data for valid players
          const sheetData = sportPlayers.map(p => ({
              "Player Name": p.name,
              [`${sport} Category`]: p.ratings[sport],
              "Contact No": p.contactNo || ""
          }));
