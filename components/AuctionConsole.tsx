...
const AuctionConsole: React.FC<AuctionConsoleProps> = ({ 
  ...
  // Tab 3: Global Distributor State
  ...
  // Initialize Distribute Limits & Accordion
  useEffect(() => {
  ...
  const getTotalRequests = () => {
      let total = 0;
      Object.values(fillRequests).forEach(sportReqs => Object.values(sportReqs).forEach(count => total += count));
      return total;
  };
  ...
  const handlePreviewBatch = () => {
      ...
      if (missingCount > 0) { setBatchError(`Could only find ${selectedBatch.length} players. ${missingCount} requests could not be filled.`); } else { setBatchError(null); }
  ...
  // --- FEATURE: GLOBAL DISTRIBUTOR (CORE LOGIC) ---
  const handleDistributePreview = () => {
    let availableLotteryPlayers = shuffleArray([...lotteryStats.pool]);
    const distribution: Record<string, Player[]> = {};
    teams.forEach(t => distribution[t.name] = []);
    
    // Deep copy teams for simulation tracking
    ...
    // Iterate Per Sport Request
    for (const sport of sports) {
        const limit = distributeLimits[sport] || 0;
        if (limit <= 0) continue;

        // Rounds: Distribute 'limit' players of 'sport' to EVERY team
        for (let round = 0; round < limit; round++) {
            for (const team of simTeams) {
                ...
                if (candidateIndex !== -1) {
                    ...
                    team.tempCount++;
                    // Update stats for all sports this player plays
                    sports.forEach(s => {
  ...
  // --- AUCTION SALE & CORRECTION HANDLERS ---
  useEffect(() => {
                         ...
                         <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 mb-8">
                             {Object.keys(distributionPreview).length === 0 ? (
                                 <div>
                                     ...
                                     <p className="text-xs text-slate-400 mb-6">Specify how many players of each sport should be assigned to <strong className="text-white">EVERY TEAM</strong> in this round from the Lottery pool.</p>
