...
import { Search, Gavel, Shuffle, Sparkles, X, ChevronRight, Lock, Ban, User, ArrowRight, Minus, Plus, Star, Scale, PenTool, CheckCircle, AlertTriangle, Clock, AlertOctagon, Wallet, Play, Target, Users, RefreshCw } from 'lucide-react';
...
const AuctionConsole: React.FC<AuctionConsoleProps> = ({ 
  ...
  const handlePreviewBatch = () => {
      ...
      const availablePool = shuffleArray<Player>(
          players.filter(p => !p.team && (p.status === 'available' || p.status === 'unsold'))
      );
      ...
      if (selectedBatch.length === 0) {
          setBatchError("No eligible players found for criteria.");
          return;
      }
                        ...
                        {!batchPreview ? (
                                    ...
                                    <div className="space-y-6">
                                        {selectedTargetTeamData ? (
                                            <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800">
                                                ...
                                                >
                                                    <Play className="w-4 h-4 fill-current" /> Preview Batch
                                                </button>
                                            </div>
                                        ) : (
                            ...
                            <div className="max-w-3xl mx-auto bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
                                <div className="bg-slate-900 p-4 border-b border-slate-800 flex items-center justify-between">
                                    ...
                                    <div className="text-sm font-bold text-slate-400">Target: <span className="text-emerald-400">{targetFillTeam}</span></div>
                                </div>
