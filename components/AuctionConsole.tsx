...
import { Search, Gavel, Shuffle, Sparkles, X, ChevronRight, Lock, Ban, User, ArrowRight, Minus, Plus, Star, Scale, PenTool, CheckCircle, AlertTriangle, Clock, AlertOctagon, Wallet, Play, Target, Users, RefreshCw, Dice5, ChevronDown, ChevronUp } from 'lucide-react';
...
const AuctionConsole: React.FC<AuctionConsoleProps> = ({ 
  ...
  // Refactored: Map Sport -> Category -> Count requested
  ...
  const updateFillRequest = (sport: string, category: string, delta: number) => {
      setFillRequests(prev => {
          ...
          // Check availability limit? Optional, but good UI. 
          // Note: We won't block going up, but we show the limit visually.
          
          return {
              ...prev,
              [sport]: { ...sportReqs, [category]: next }
          };
      });
  };
  ...
  const handlePreviewBatch = () => {
      ...
      // 1. Flatten Requests into a Demand List
      // e.g. [{sport: 'Cricket', category: 'A'}, {sport: 'Cricket', category: 'A'}, {sport: 'Badminton', category: 'B'}]
      ...
      for (const demand of demands) {
          // Find first player who matches criteria AND isn't used
          const candidate = pool.find(p => 
              !usedIds.has(p.id) && 
              p.ratings[demand.sport] === demand.category
          );
          ...
          if (candidate) {
              ...
              selectedBatch.push({
                  player: candidate,
                  covered: [`${demand.sport} (Grade ${demand.category})`]
              });
          } else {
              missingCount++;
          }
      ...
      if (selectedBatch.length === 0) {
          setBatchError(`No eligible players found matching your specific Grade criteria in ${targetPoolType} pool.`);
          return;
      }

      if (missingCount > 0) {
          setBatchError(`Could only find ${selectedBatch.length} players. ${missingCount} requests could not be filled (insufficient stock).`);
      } else {
          setBatchError(null);
      }
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                                    ...
                                    <div className="md:col-span-4 lg:col-span-3 space-y-6">
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <button onClick={() => setTargetPoolType('LOTTERY')} className={`flex-1 py-2 rounded text-sm font-bold flex items-center justify-center gap-2 ${targetPoolType === 'LOTTERY' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
                                                        <Dice5 className="w-4 h-4"/> Lottery
                                                    </button>
                                                </div>
                                                <div className="text-center mt-2 text-[10px] uppercase font-bold text-slate-500 tracking-wider">
                                                    Total Available: <span className="text-white">{poolAvailability.total}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 gap-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                                                {teams.map(t => (
                                                    <button 
                                                        ...
                                                        className={`p-3 rounded-xl border text-left transition-all ${targetFillTeam === t.name ? 'bg-emerald-600 border-emerald-500 text-white ring-2 ring-emerald-500/30' : 'bg-slate-900 border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                                                    >
                                                        <div className="font-bold text-sm truncate">{t.name}</div>
                                                        <div className="text-xs opacity-70 mt-1 flex justify-between">
                                                            <span>Size: {t.playerCount}</span>
                                                            <span className="font-mono">{t.disposableBalance}</span>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>
                                        ...
                                        <button 
                                            ...
                                            disabled={getTotalRequests() === 0}
                                            className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/20"
                                        >
                                            <Play className="w-5 h-5 fill-current" /> Preview Batch
                                        </button>
                                            ...
                                            <div className="bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden">
                                                <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
                                                    <h4 className="font-bold text-white flex items-center gap-2">
                                                        ...
                                                        <span className="truncate">{selectedTargetTeamData.name} Wishlist</span>
                                                    </h4>
                                                        ...
                                                        return (
                                                            <div key={sport} className={`border rounded-xl overflow-hidden ${colors.border} bg-slate-900/30`}>
                                                                ...
                                                                <button 
                                                                    ...
                                                                    className={`w-full flex items-center justify-between p-3 ${colors.soft} hover:brightness-110 transition-all`}
                                                                >
                                                                    <div className="flex items-center gap-3">
                                                                        <span className={`font-black uppercase text-sm ${colors.text}`}>{sport}</span>
                                                                        <span className="text-[10px] font-bold bg-black/20 px-2 py-0.5 rounded text-white/70">
                                                                            Team has: {teamSportStats.total}
                                                                        </span>
                                                                    </div>
                                                                            ...
                                                                            return (
                                                                                <div key={cat} className="flex items-center justify-between p-3 hover:bg-slate-800/50 transition-colors">
                                                                                    <div className="flex items-center gap-4 w-1/3">
                                                                                        <span className="text-sm font-bold text-slate-300 w-16">Grade {cat}</span>
                                                                                    ...
                                                                                    <div className="w-1/3 text-center">
                                                                                        <span className={`text-[10px] font-bold uppercase tracking-wider ${availCount > 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                                                                                            Avail: {availCount}
                                                                                        </span>
                                                                                    </div>
                                                                                    ...
                                                                                    <div className="flex items-center gap-2 w-1/3 justify-end">
                                                                                        <button 
                                                                                            onClick={() => updateFillRequest(sport, cat, -1)}
                                                                                            className="w-7 h-7 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-lg border border-slate-700 transition-colors"
                                    ...
                                    <table className="w-full text-left text-sm">
                                        <thead className="bg-slate-900 text-slate-500 uppercase text-xs font-bold sticky top-0">
                                            <tr>
                                                ...
                                                <th className="p-4">Selection Criteria</th>
                                                ...
                                                <tr key={idx}>
                                                    ...
                                                    <td className="p-4">
                                                        <div className="flex flex-wrap gap-2">
                                                            {item.covered.map((reason, i) => (
                                                                <span 
                                                                    key={i} 
                                                                    className="px-2 py-1 rounded text-xs font-bold border bg-indigo-900/30 text-indigo-300 border-indigo-500/30"
                                                                >
                                                                    {reason}
                                                                </span>
                                                            ))}
