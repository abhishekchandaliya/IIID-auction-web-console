...
import { Search, Plus, Edit2, Trash2, X, Save, User, Phone, Gavel, Dice5, Download, Layers } from 'lucide-react';
...
interface PlayerManagerProps {
  ...
  sports: string[];
  categories: string[];
  onAdd: (player: Omit<Player, 'id' | 'team' | 'price' | 'status' | 'captainFor' | 'auctionType'>) => void;
  onUpdate: (player: Player) => void;
  onDelete: (id: number) => void;
  onBulkUpdateType: (ids: number[], type: 'LIVE' | 'LOTTERY') => void;
}

const PlayerManager: React.FC<PlayerManagerProps> = ({ players, sports, categories, onAdd, onUpdate, onDelete, onBulkUpdateType }) => {
  ...
  const filteredPlayers = useMemo(() => {
    if (!searchTerm) return players.slice(0, 100); 
  ...
  const openAddModal = () => {
    ...
    setFormData({ name: "", contactNo: "", ratings: initialRatings, auctionType: 'LIVE' });
    setIsModalOpen(true);
  };
  ...
  const handleBulkAction = (type: 'LIVE' | 'LOTTERY') => {
      ...
      if (count === 0) return;
      if (confirm(`Set ${count} filtered players to ${type} pool?`)) {
  ...
  const handleExportList = (type: 'LIVE' | 'LOTTERY') => {
      ...
      const data = list.map(p => {
          const row: any = { ID: p.id, Name: p.name, Type: p.auctionType, Status: p.status };
  ...
  return (
    <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 mt-8">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="flex gap-2 w-full md:w-auto">
              <div className="relative flex-1 md:w-64">
                  ...
                  <input 
                    type="text" 
                    placeholder="Search (e.g. 'Grade C')" 
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-4">
          <div className="flex-1 flex gap-2 items-center bg-slate-900 p-2 rounded-xl border border-slate-800">
              <span className="text-[10px] font-bold text-slate-500 uppercase px-2">Batch Edit ({filteredPlayers.length})</span>
              <button onClick={() => handleBulkAction('LIVE')} className="flex items-center gap-1 px-3 py-1.5 bg-indigo-900/30 text-indigo-300 border border-indigo-500/30 rounded text-xs font-bold hover:bg-indigo-900/50"><Gavel className="w-3 h-3"/> Set LIVE</button>
              <button onClick={() => handleBulkAction('LOTTERY')} className="flex items-center gap-1 px-3 py-1.5 bg-amber-900/30 text-amber-300 border border-amber-500/30 rounded text-xs font-bold hover:bg-amber-900/50"><Dice5 className="w-3 h-3"/> Set LOTTERY</button>
          </div>
          <div className="flex gap-2">
              <button onClick={() => handleExportList('LIVE')} className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-bold flex items-center gap-2"><Download className="w-3 h-3"/> Auction List</button>
              <button onClick={() => handleExportList('LOTTERY')} className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-xs font-bold flex items-center gap-2"><Download className="w-3 h-3"/> Lottery List</button>
          </div>
      </div>
          ...
          <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-800">
                      ...
                      <tr key={p.id} className="hover:bg-slate-900/50 transition-colors">
                          ...
                          <td className="p-3 text-center">
                              <button 
                                ...
                                className={`px-2 py-1 rounded text-[10px] font-bold uppercase flex items-center gap-1 mx-auto border transition-all ${
                                  p.auctionType === 'LIVE' 
                  ...
                  {filteredPlayers.length === 0 && (
                      <tr><td colSpan={5 + sports.length} className="p-8 text-center text-slate-500">No players found matching filter</td></tr>
                  )}
              </tbody>
          </table>
      </div>
              ...
              <div className="bg-slate-900 w-full max-w-lg rounded-2xl border border-slate-700 shadow-2xl flex flex-col max-h-[90vh]">
                  ...
                  <form onSubmit={handleSubmit} className="p-6 overflow-y-auto custom-scrollbar">
                      <div className="space-y-4">
                          ...
                          <div>
                              <label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Pool Type</label>
                              <div className="flex gap-2">
                                  <button type="button" onClick={() => setFormData({...formData, auctionType: 'LIVE'})} className={`flex-1 py-2 rounded-lg border text-sm font-bold flex items-center justify-center gap-2 ${formData.auctionType === 'LIVE' ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
                                      <Gavel className="w-4 h-4" /> Live Auction
                                  </button>
                                  <button type="button" onClick={() => setFormData({...formData, auctionType: 'LOTTERY'})} className={`flex-1 py-2 rounded-lg border text-sm font-bold flex items-center justify-center gap-2 ${formData.auctionType === 'LOTTERY' ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
